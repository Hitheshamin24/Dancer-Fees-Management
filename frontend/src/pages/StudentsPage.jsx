...
export default function StudentsPage() {
  ...
  const [input, setInput] = useState(""); // single input for add + search
  ...
  async function handleAdd(e) {
    e.preventDefault();
    if (!input.trim() || !clerkId) return;

    // Check if a student with that name already exists
    const exists = students.some(
      (s) => s.name.toLowerCase() === input.trim().toLowerCase()
    );
    if (exists) return; // prevent adding duplicates

    try {
      await addStudent(input.trim(), clerkId);
      setInput(""); // clear after add
      load();
    } catch (e) {
      setError(e.message);
    }
